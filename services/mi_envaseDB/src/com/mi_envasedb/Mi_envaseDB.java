    ...
    implements DataServiceManagerAccess, LiveDataService
{
    ...
    public com.mi_envasedb.data.Cliente getClienteById(Integer id, PagingOptions pagingOptions) {
        List<com.mi_envasedb.data.Cliente> rtn = ((List<com.mi_envasedb.data.Cliente> ) dsMgr.invoke(taskMgr.getQueryTask(), (Mi_envaseDBConstants.getClienteByIdQueryName), id, pagingOptions));
        if (rtn.isEmpty()) {
            return null;
        } else {
            return rtn.get(0);
